CLEAN_TEMPERATURE<-function(states_data=NULL,temperature_dataset=NULL,arimax=FALSE, US_STATE=3, auto=FALSE, my_n_ahead=1,ES27=TRUE,ES64=FALSE){
  ...
  ###############################
  # Get the Errors by each week #
  ###############################
  forecasts<-data.frame(list_all_preds[[1]]) # get log1p of the forecasts
  colnames(forecasts)[1] <- "Dates"
  cases_by_week<-data.frame(states_data[[US_STATE]]$target_end_date,states_data[[US_STATE]]$cases) # get the cases for this state
  colnames(cases_by_week)[1] <- "Dates"
  forecasts_and_cases<-merge(forecasts,cases_by_week, by="Dates") #merge
  colnames(forecasts_and_cases) <- c("Dates","Forecasts","Cases")
  forecasts_and_cases$absolute_error <-(expm1(forecasts_and_cases$Forecasts) - forecasts_and_cases$Cases) # back transform the forecasts and calculate the error
  my_errors <- data.frame(forecasts_and_cases$Dates, forecasts_and_cases$absolute_error)
  colnames(my_errors) <- c("forecast_date","error")
  ########################
